...
The application SHALL render a schedule grid for a selected evening. Columns represent stage events that are active on the selected evening. For stages with a single event on the evening, one column is rendered (as before). For stages with multiple events on the evening, one column is rendered **per event**, each with its own time axis contribution. Columns for events belonging to the same stage SHALL appear adjacently in stage-definition order, with the stage name displayed as a grouped super-header spanning all of its event columns for that evening. Sequential stage event columns have rows representing time slots derived from each event's start time, end time, and slot duration. Simultaneous stage columns remain unchanged (single cell, max 3 DJs, no event subdivision). Stage/event columns SHALL appear left-to-right in stage-definition order, with each stage's events ordered by eventIndex. When a stage has an assigned color, all of its event column headers and occupied cells SHALL display the same color accent.

#### Scenario: Grid renders columns for active stages on selected evening
- **WHEN** the organizer selects an evening (e.g., "Friday")
- **THEN** the grid renders one column per stage-event for each stage active on that evening
- **THEN** a stage with one event contributes one column; a stage with two events contributes two columns
- **THEN** stages not active on that evening are not shown
- **THEN** columns appear left-to-right in stage-definition order, events within a stage in eventIndex order

#### Scenario: Multi-event stage shows grouped column headers
- **WHEN** a stage has two events on the selected evening (e.g., "Afternoon Set" and "Evening Set")
- **THEN** the grid renders two adjacent columns, one per event
- **THEN** a super-header spanning both columns displays the stage name
- **THEN** each column header displays the event label (or auto-label "Set 1", "Set 2" if unlabeled)

#### Scenario: Grid renders time slot rows for sequential stage events
- **WHEN** a sequential stage event has start time, end time, and slot duration configured
...
#### Scenario: Simultaneous stage column spans full height (unchanged)
...
- **THEN** all of that stage's event column headers SHALL display the same color accent
- **THEN** stages without a color SHALL render column headers with default neutral styling

### Requirement: Assign DJ to a slot
The application SHALL allow the organizer to assign a DJ from the unscheduled pool to an empty sequential grid slot or to an open position in a simultaneous stage cell. Slot identity includes `(stageId, evening, eventIndex, slotIndex)` for sequential stages.

#### Scenario: Click empty sequential slot to assign
- **WHEN** the organizer clicks an empty sequential slot cell in any event column
- **THEN** a picker opens listing all DJs who have no assignment anywhere in the lineup
- **THEN** selecting a DJ assigns them to that slot (including correct eventIndex) and removes them from the pool globally
...
- **WHEN** a DJ has been assigned to any slot or simultaneous position (on any evening, on any stage, in any event)
...
- **THEN** the grid updates to show only the stages and stage-events for that evening
...
The active stage/event context in the lineup builder SHALL only change when the user explicitly clicks a slot or event cell in the grid. Auto-advance after an assignment SHALL only move within the same stage AND same event (same `stageId` and same `eventIndex`). No background process, automatic navigation, or derived state change SHALL silently switch the active context to a different stage or event without user interaction.

#### Scenario: Auto-advance after assignment stays within the same stage event
- **WHEN** the user assigns a DJ to a sequential slot and the system auto-advances to the next empty slot
- **THEN** the next slot SHALL be within the same stage AND same event (same stageId and eventIndex)
- **THEN** the active stage/event context SHALL NOT change to a different stage or event column

#### Scenario: Auto-advance stops when no remaining empty slot in the stage event
- **WHEN** the user assigns a DJ to the last empty sequential slot within the active stage event
- **THEN** the active slot SHALL remain on the just-assigned slot (or the last slot in that event)
- **THEN** NO automatic cross-stage or cross-event jump SHALL occur
- **THEN** the user must explicitly click a slot in another stage or event column to move there
...
- **THEN** the pool panel lists only DJs who have no assignment anywhere in the lineup (sequential or simultaneous, any evening, any stage, any event)
