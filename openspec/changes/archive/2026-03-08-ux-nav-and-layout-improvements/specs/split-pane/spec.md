...
Both `SubmissionsView` (list ↔ detail panel) and `LineupView` (grid ↔ DJ selection panel) SHALL use the `SplitPane` component for their two-pane layout instead of bespoke CSS split classes.
