...
#### Scenario: Occupied sequential slot displays DJ name
- **WHEN** a DJ has been assigned to a sequential slot
...
The application SHALL provide a navigation control to switch between the convention evenings. Only evenings that have at least one active stage (sequential or simultaneous) SHALL be shown.
