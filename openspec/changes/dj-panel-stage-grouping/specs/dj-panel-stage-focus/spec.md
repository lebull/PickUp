...
**Reason**: Replaced by the stage focus grouping approach, which surfaces preference rank rather than just filtering by presence.
**Migration**: Remove `selectedStages: Set<string>` state and multi-toggle filter UI; replace with `focusStage: string | null` state and grouping render logic.
