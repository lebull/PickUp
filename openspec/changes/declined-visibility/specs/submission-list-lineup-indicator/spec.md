...
The submission list SHALL display visual indicators on each row reflecting the row's current state. Submission-quality issue badges (discarded, declined, duplicate-name) SHALL remain in the DJ Name column. The in-lineup stage assignment badge SHALL be displayed in the dedicated Stage Assignment column (visible only when the "View stage assignments" toggle is on). State is determined by priority order: `discarded` > `declined` > `in-lineup` > `duplicate-name` > `none`. Matching for `in-lineup` SHALL be performed by `submissionNumber`. Matching for `discarded` SHALL check `project.discardedSubmissions`. Matching for `declined` SHALL check whether the submission currently holds a DJ slot assignment with `acceptanceStatus` equal to `'no'`. Matching for `duplicate-name` SHALL check whether any other non-discarded submission shares the same `djName`. When a submission is in the lineup and the assigned stage has a color and the stage-assignment toggle is on, the row SHALL be tinted with that stage's palette color.
...
- **THEN** no other state badge (declined, in-lineup, duplicate-name) SHALL be shown on that row

#### Scenario: Declined row shows declined badge in DJ Name column
- **WHEN** a submission has an active lineup assignment and that assignment has `acceptanceStatus` of `'no'` AND the submission is not discarded
- **THEN** the row SHALL display a `declined` badge in the DJ Name column
- **THEN** the declined badge SHALL take precedence over in-lineup and duplicate-name badges

#### Scenario: In-lineup row shows stage badge in Stage Assignment column
- **WHEN** a submission's `submissionNumber` matches an assignment in the current project's lineup AND the submission is not discarded AND does not currently qualify as declined AND the stage-assignment toggle is on
